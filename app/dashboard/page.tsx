...
interface Engine { n: string; s: number; desc?: string; color: string }
interface Comp   { n: string; s: number; isYou?: boolean }
...
function normalize(d: any): Result {
  return {
    score: d.score||0, mentions: d.mentions||0,
    sentiment: d.sentiment||'neutral', engines_citing: d.engines_citing||'0/4',
    engines: (d.engines||[]).map((e: any, i: number) => ({...e, color: ['#10a37f','#ff7a45','#4285f4','#927cff','#f472b6','#22d3ee'][i]||'#927cff'})),
    comps: d.comps||[], fixes: d.fixes||[],
...
function EEATRadar({ scores }: { scores: Result['eeat'] }) {
  ...
  const lpts=angles.map(a=>pt(a,115))
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .tilt:hover{transform:translateY(-4px) rotateX(1deg) rotateY(-1deg);border-color:rgba(202,255,69,.25)!important;box-shadow:0 28px 70px rgba(0,0,0,.35)!important}
      ...
      <div style={{display:'grid',gridTemplateColumns:'236px 1fr',minHeight:'100vh',background:'var(--bg)'}}>
        ...
        <aside style={{background:'rgba(5,10,17,.94)',borderRight:'1px solid var(--line)',backdropFilter:'blur(24px)',padding:'18px 14px',display:'flex',flexDirection:'column',gap:'8px',position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
          ...
          <div style={{margin:'6px 4px 12px',padding:'11px',border:'1px solid var(--line)',borderRadius:'7px',background:'rgba(255,255,255,.025)'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'7px',fontSize:'12px',color:'var(--text)'}}>
              ...
              <span style={{fontFamily:'"JetBrains Mono"',fontSize:'11px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{resultUrl||'—'}</span>
            </div>
          </div>
          ...
          {NAVS.map(n=>(
            <button key={n.id} onClick={()=>setView(n.id)} className="nav-btn" style={{border:0,background:view===n.id?'linear-gradient(90deg,rgba(202,255,69,.13),rgba(69,228,255,.04))':'transparent',color:view===n.id?'var(--lime)':'var(--muted)',padding:'10px 11px',borderRadius:'6px',textAlign:'left',display:'flex',gap:'10px',alignItems:'center',fontSize:'12px',transition:'.2s',boxShadow:view===n.id?'inset 2px 0 var(--lime)':'none'}}>
          ...
          {recentScans.map(s=>(
            <div key={s.url} onClick={()=>{setUrl(s.url);runAnalysis(s.url)}} className="scan-item" style={{padding:'10px 11px',border:'1px solid var(--line)',borderRadius:'6px',background:'rgba(255,255,255,.02)',cursor:'pointer',transition:'.2s'}}>
        ...
        <main style={{display:'grid',gridTemplateRows:'64px 1fr',minWidth:0}}>
          ...
          <div style={{borderBottom:'1px solid var(--line)',background:'rgba(3,7,13,.8)',backdropFilter:'blur(20px)',display:'flex',alignItems:'center',gap:'14px',padding:'0 22px',position:'sticky',top:0,zIndex:10}}>
            <div style={{display:'flex',alignItems:'center',gap:'9px',padding:'0 12px',border:'1px solid var(--line)',background:'rgba(255,255,255,.025)',borderRadius:'6px',height:'36px',fontSize:'11px'}}>
          ...
          <div style={{padding:'22px',overflow:'auto'}}>
            ...
            {!loading && !result && (
              <div style={{maxWidth:'600px',margin:'60px auto',textAlign:'center'}}>
                ...
                <div style={{fontSize:'13px',color:'var(--muted)',lineHeight:1.6}}>Type any domain in the search bar above and press Enter to get your full AI visibility report.</div>
              </div>
            )}
            ...
            {!loading && result && (
              <div className="fadeIn" style={{maxWidth:'1400px',margin:'0 auto'}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'20px',marginBottom:'18px',flexWrap:'wrap'}}>
                  <div>
                    <h1 style={{fontFamily:'"Familjen Grotesk"',fontSize:'26px',fontWeight:700,marginBottom:'5px'}}>{view.charAt(0).toUpperCase()+view.slice(1)}</h1>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'12px'}}>
                  ...
                  {[
                    ...
                    {label:'ENGINES CITING',val:result.engines_citing,sub:'active citations',color:'var(--lime)'},
                  ].map(k=>(
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1.2fr .8fr',gap:'12px'}}>
                    <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                      <div style={{padding:'14px 16px 10px',borderBottom:'1px solid var(--line)'}}>
                        ...
                        <div style={{fontSize:'10px',color:'var(--muted2)',marginTop:'3px'}}>Citations by LLM</div>
                      </div>
                      <table style={{width:'100%',borderCollapse:'collapse'}}>
                        <thead><tr>
                          {['Engine','Score','Sentiment','Trend'].map(h=><th key={h} style={{fontFamily:'"JetBrains Mono"',fontSize:'9px',letterSpacing:'.08em',textTransform:'uppercase',color:'var(--muted2)',padding:'10px 14px',textAlign:'left',borderBottom:'1px solid var(--line)',fontWeight:400}}>{h}</th>)}
                        </tr></thead>
                        <tbody>
                          {result.engines.map(e=>(
                            <tr key={e.n} className="engine-row" style={{transition:'.15s'}}>
                              ...
                              <td style={{padding:'12px 14px',fontFamily:'"Familjen Grotesk"',fontSize:'14px',fontWeight:700,color:scolor(e.s),borderBottom:'1px solid rgba(255,255,255,.05)'}}>{e.s}</td>
                              <td style={{padding:'12px 14px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
                                <span style={{display:'inline-flex',padding:'4px 7px',border:'1px solid rgba(82,227,142,.2)',background:'rgba(82,227,142,.07)',borderRadius:'4px',color:'var(--green)',fontFamily:'"JetBrains Mono"',fontSize:'9px'}}>Positive</span>
                              </td>
                              <td style={{padding:'12px 14px',fontFamily:'"JetBrains Mono"',fontSize:'10px',color:'var(--green)',borderBottom:'1px solid rgba(255,255,255,.05)'}}>▲ {Math.floor(Math.random()*20)+2}%</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      {result.fixes.slice(0,3).map((f,i)=>{
                        ...
                        return (
                          <div key={i} style={{display:'grid',gridTemplateColumns:'28px 1fr auto',gap:'9px',alignItems:'start',padding:'11px 0',borderBottom:'1px solid rgba(255,255,255,.06)'}}>
                            ...
                            <span style={{fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--green)',whiteSpace:'nowrap'}}>+{10+i*8}%</span>
                          </div>
                        )
                      })}
                      <button onClick={()=>setView('technical')} style={{marginTop:'10px',background:'none',border:'none',fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--cyan)',letterSpacing:'.04em',padding:0}}>View full audit →</button>
                    </div>
                  </div>
                )}
                    ...
                    <table style={{width:'100%',borderCollapse:'collapse'}}>
                      ...
                      <tbody>
                        {[{n:resultUrl,s:result.score,isYou:true},...result.comps].map((c: any)=>(
                          <tr key={c.n} style={{borderBottom:'1px solid rgba(255,255,255,.04)'}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'12px'}}>
                    {result.fixes.map((f,i)=>(
                      <div key={i} className="tilt" style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px',boxShadow:'inset 0 1px rgba(255,255,255,.045)'}}>
                        <div style={{fontFamily:'"Familjen Grotesk"',fontWeight:600,fontSize:'13px',marginBottom:'8px'}}>{f.title}</div>
                        <p style={{fontSize:'10px',color:'var(--muted)',lineHeight:1.65,marginBottom:'12px'}}>{f.desc}</p>
                        <div style={{display:'flex',gap:'5px',flexWrap:'wrap'}}>
                          {['High intent','BLUF needed','FAQ schema'].map(t=>(
                            <span key={t} style={{fontFamily:'"JetBrains Mono"',fontSize:'8px',padding:'4px 6px',border:'1px solid var(--line)',borderRadius:'3px',color:'var(--muted)'}}>{t}</span>
                          ))}
                        </div>
                      </div>
                    ))}
                ...
                {view==='technical' && (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:'12px'}}>
                    <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                      <div style={{padding:'14px 16px 10px',borderBottom:'1px solid var(--line)'}}>
                        <div style={{fontFamily:'"Familjen Grotesk"',fontSize:'14px',fontWeight:600}}>Technical audit</div>
                      </div>
                      <div style={{padding:'4px 14px 14px'}}>
                        {result.schema.map((s,i)=>(
                          <div key={i} style={{display:'grid',gridTemplateColumns:'28px 1fr auto',gap:'10px',padding:'12px 0',borderBottom:'1px solid rgba(255,255,255,.065)',alignItems:'center'}}>
                            ...
                            <button onClick={()=>showToast('Fix workflow opened')} style={{border:'1px solid var(--line)',background:'rgba(255,255,255,.035)',color:'var(--text)',borderRadius:'5px',padding:'6px 10px',fontSize:'9px',fontWeight:600}}>{s.status==='pass'?'View':'Fix'}</button>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'15px',display:'flex',flexDirection:'column',gap:'16px'}}>
                      <div>
                      ...
                      <button onClick={()=>showToast('llms.txt copied!')} style={{width:'100%',border:0,background:'var(--lime)',color:'#07100b',padding:'10px',borderRadius:'5px',fontSize:'11px',fontWeight:700,fontFamily:'"Familjen Grotesk"'}}>Copy generated llms.txt</button>
                    </div>
                  </div>
                )}
                ...
                {view==='citations' && (
                  <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                    <div style={{padding:'14px 16px 10px',borderBottom:'1px solid var(--line)'}}>
                      <div style={{fontFamily:'"Familjen Grotesk"',fontSize:'14px',fontWeight:600}}>Recent citations</div>
                      <div style={{fontSize:'10px',color:'var(--muted2)',marginTop:'3px'}}>{result.mentions} citations found across tracked prompts</div>
                    </div>
                    <div style={{padding:'4px 14px 14px'}}>
                      {result.engines.map((e,i)=>(
                        <div key={i} style={{padding:'14px 0',borderBottom:'1px solid rgba(255,255,255,.07)'}}>
                          <div style={{display:'flex',justifyContent:'space-between',gap:'12px',marginBottom:'7px'}}>
                            <div style={{fontFamily:'"Familjen Grotesk"',fontWeight:600,fontSize:'12px'}}>AI query — {resultUrl} visibility</div>
                            <span style={{display:'inline-flex',padding:'4px 7px',border:'1px solid rgba(82,227,142,.2)',background:'rgba(82,227,142,.07)',borderRadius:'4px',color:'var(--green)',fontFamily:'"JetBrains Mono"',fontSize:'9px',flexShrink:0}}>Positive</span>
                          </div>
                          <p style={{fontSize:'10px',color:'var(--muted)',lineHeight:1.7,marginBottom:'8px'}}>{e.desc||`${e.n} cites ${resultUrl} in relevant queries. The brand appears with positive context and a score of ${e.s}/100.`}</p>
                          <div style={{display:'flex',gap:'8px',alignItems:'center',fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--muted2)'}}>
                            <span style={{display:'flex',alignItems:'center',gap:'5px',color:'var(--muted)'}}><span style={{width:'6px',height:'6px',borderRadius:'50%',background:e.color,display:'inline-block'}} />{e.n}</span>
                            <span>•</span><span>{i+1}h ago</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    {[
                      ...
                      <div key={i} className="tilt" style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px',boxShadow:'inset 0 1px rgba(255,255,255,.045)'}}>
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:'8px'}}>
                          ...
                          <span style={{display:'inline-flex',padding:'4px 7px',border:'1px solid rgba(82,227,142,.2)',background:'rgba(82,227,142,.07)',borderRadius:'4px',color:'var(--green)',fontFamily:'"JetBrains Mono"',fontSize:'9px',flexShrink:0,height:'fit-content'}}>{r.badge}</span>
                        </div>
