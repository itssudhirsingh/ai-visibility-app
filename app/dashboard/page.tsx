...
function DashboardInner() {
  ...
  return (
    <>
      ...
      <div style={{display:'grid',gridTemplateColumns:'236px 1fr',minHeight:'100vh',background:'var(--bg)'}}>
        ...
        {/* ── MAIN ── */}
        <main style={{display:'grid',gridTemplateRows:'64px 1fr',minWidth:0}}>
          {/* Topbar */}
          <div style={{borderBottom:'1px solid var(--line)',background:'rgba(3,7,13,.8)',backdropFilter:'blur(20px)',display:'flex',alignItems:'center',gap:'14px',padding:'0 22px',position:'sticky',top:0,zIndex:10}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                          ...
                          <tbody>
                            {result.engines.map(e=>(
                              <tr key={e.n} className="engine-row" style={{transition:'.15s'}}>
                                ...
                                <td style={{padding:'11px 14px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
                                  <span className="pill" style={{border:`1px solid ${e.sentiment==='negative'?'rgba(255,116,116,.2)':'rgba(82,227,142,.2)'}`,background:e.sentiment==='negative'?'rgba(255,116,116,.07)':'rgba(82,227,142,.07)',color:e.sentiment==='negative'?'var(--red)':'var(--green)'}}>
                                    {e.sentiment||'positive'}
                                  </span>
                                </td>
                                <td style={{padding:'11px 14px',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
                                  <span className="pill" style={{border:`1px solid ${e.s>=65?'rgba(82,227,142,.2)':'rgba(255,196,92,.2)'}`,background:e.s>=65?'rgba(82,227,142,.07)':'rgba(255,196,92,.07)',color:e.s>=65?'var(--green)':'var(--amber)'}}>
                                    {e.status||(e.s>=65?'CITED':'LOW')}
                                  </span>
                                </td>
                              </tr>
                            ))}
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'12px'}}>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px'}}>
                        ...
                        {result.bluf.headline && (
                          <div style={{padding:'8px 10px',background:'rgba(255,255,255,.03)',borderRadius:'5px',fontSize:'10px',color:'var(--muted)',lineHeight:1.6,marginBottom:'10px',border:'1px solid var(--line)',fontStyle:'italic'}}>
                            "{result.bluf.headline}"
                          </div>
                        )}
                        {result.bluf.issues.length>0 ? (
                          <div style={{display:'flex',flexDirection:'column',gap:'5px',marginBottom:'10px'}}>
                            {result.bluf.issues.slice(0,3).map((issue,i)=>(
                              <div key={i} style={{display:'flex',gap:'6px',alignItems:'flex-start',fontSize:'9px',color:'var(--muted)'}}>
                                <span style={{color:'var(--amber)',flexShrink:0,marginTop:'1px'}}>!</span>{issue}
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div style={{fontSize:'9px',color:'var(--green)',display:'flex',alignItems:'center',gap:'5px',marginBottom:'10px'}}>
                            <span>✓</span> Homepage passes BLUF check
                          </div>
                        )}
                        <div style={{height:'4px',background:'rgba(255,255,255,.06)',borderRadius:'9px',overflow:'hidden'}}>
                          <div style={{height:'100%',width:`${result.bluf.score}%`,background:scolor(result.bluf.score),borderRadius:'9px',transition:'width 1s'}} />
                        </div>
                      </div>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
                          ...
                          <span className="pill" style={{border:`1px solid ${result.llms_txt.exists?'rgba(82,227,142,.25)':'rgba(255,116,116,.25)'}`,background:result.llms_txt.exists?'rgba(82,227,142,.08)':'rgba(255,116,116,.08)',color:result.llms_txt.exists?'var(--green)':'var(--red)'}}>
                            {result.llms_txt.exists?'FOUND':'MISSING'}
                          </span>
                        </div>
                        <div style={{display:'flex',flexDirection:'column',gap:'7px',marginBottom:'12px'}}>
                          {[
                            {label:'File exists at /llms.txt',     pass:result.llms_txt.exists},
                            {label:'Valid format',                  pass:result.llms_txt.valid},
                            {label:'Crawlers correctly configured', pass:result.llms_txt.exists&&result.llms_txt.valid},
                          ].map((chk,i)=>(
                            <div key={i} style={{display:'flex',alignItems:'center',gap:'8px',fontSize:'10px',color:chk.pass?'var(--green)':'var(--muted)'}}>
                              <span style={{width:'16px',height:'16px',borderRadius:'3px',background:chk.pass?'rgba(82,227,142,.1)':'rgba(255,255,255,.04)',border:`1px solid ${chk.pass?'rgba(82,227,142,.25)':'rgba(255,255,255,.08)'}`,display:'grid',placeItems:'center',fontSize:'9px',color:chk.pass?'var(--green)':'var(--muted2)',flexShrink:0}}>
                                {chk.pass?'✓':'×'}
                              </span>
                              {chk.label}
                            </div>
                        ...
                        <button onClick={()=>setView('technical')} style={{background:'none',border:'none',fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--cyan)',padding:0}}>
                          {result.llms_txt.exists?'View full file →':'Generate llms.txt →'}
                        </button>
                      </div>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
                          ...
                          {result.weekly_trend.length>1 && (
                            <span style={{fontFamily:'"JetBrains Mono"',fontSize:'10px',color:result.weekly_trend[result.weekly_trend.length-1].score>=result.weekly_trend[0].score?'var(--green)':'var(--red)'}}>
                              {result.weekly_trend[result.weekly_trend.length-1].score>=result.weekly_trend[0].score?'▲':'▼'} {Math.abs(result.weekly_trend[result.weekly_trend.length-1].score - result.weekly_trend[0].score)} pts
                            </span>
                          )}
                        </div>
                ...
                {view==='citations' && (
                  <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                    <div style={{padding:'12px 16px',border:'1px solid rgba(69,228,255,.15)',background:'rgba(69,228,255,.04)',borderRadius:'7px',fontSize:'10px',color:'var(--muted)',lineHeight:1.6}}>
                      ...
                      Per-engine mention analysis with sentiment and sample snippets showing how each AI references {resultUrl}.
                    </div>
                      ...
                      <div key={i} style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                        <div style={{padding:'14px 16px',borderBottom:'1px solid var(--line)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
                            <span className="pill" style={{border:`1px solid ${e.sentiment==='negative'?'rgba(255,116,116,.2)':'rgba(82,227,142,.2)'}`,background:e.sentiment==='negative'?'rgba(255,116,116,.07)':'rgba(82,227,142,.07)',color:e.sentiment==='negative'?'var(--red)':'var(--green)'}}>
                              {e.sentiment||'positive'} sentiment
                            </span>
                        ...
                        <div style={{padding:'14px 16px'}}>
                          {e.desc && (
                            <p style={{fontSize:'11px',color:'var(--muted)',lineHeight:1.7,marginBottom:'12px',paddingBottom:'12px',borderBottom:'1px solid rgba(255,255,255,.06)'}}>{e.desc}</p>
                          )}
                          {e.citations && e.citations.length>0 ? (
                            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                              <div style={{fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--muted2)',letterSpacing:'.08em',marginBottom:'4px'}}>CITATION SNIPPETS</div>
                              {e.citations.map((c,j)=>(
                                <div key={j} style={{padding:'10px 12px',background:'rgba(255,255,255,.025)',border:'1px solid var(--line)',borderLeft:`2px solid ${e.color}`,borderRadius:'6px',fontSize:'10px',color:'var(--muted)',lineHeight:1.7}}>
                                  "{c}"
                                </div>
                              ))}
                            </div>
                          ) : (
                            <div style={{padding:'12px',background:'rgba(255,255,255,.02)',borderRadius:'6px',fontSize:'10px',color:'var(--muted2)',fontFamily:'"JetBrains Mono"',textAlign:'center'}}>
                              No citation snippets — add <span style={{color:'var(--lime)'}}>citations[]</span> array to API response schema
                            </div>
                          )}
                        </div>
                      </div>
                ...
                {view==='probes' && (
                  <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                    <div style={{padding:'12px 16px',border:'1px solid rgba(202,255,69,.15)',background:'rgba(202,255,69,.04)',borderRadius:'7px',fontSize:'10px',color:'var(--muted)',lineHeight:1.6,marginBottom:'4px'}}>
                      ...
                      Actual prompts submitted to each AI engine during this scan, with raw response excerpts. Click any row to expand.
                    </div>
                    {result.query_probes.length>0 ? result.query_probes.map((p,i)=>(
                      <div key={i} onClick={()=>setOpenProbe(openProbe===i?null:i)} className="probe-row"
                        style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden',transition:'.15s'}}>
                        <div style={{padding:'12px 16px',display:'flex',alignItems:'center',gap:'12px'}}>
                          ...
                          <span className="pill" style={{border:`1px solid ${p.cited?'rgba(82,227,142,.2)':'rgba(255,196,92,.2)'}`,background:p.cited?'rgba(82,227,142,.07)':'rgba(255,196,92,.07)',color:p.cited?'var(--green)':'var(--amber)',flexShrink:0}}>
                            {p.cited?'CITED':'NOT CITED'}
                          </span>
                          <span style={{color:'var(--muted2)',fontSize:'10px',marginLeft:'4px'}}>{openProbe===i?'▲':'▼'}</span>
                        </div>
                        {openProbe===i && (
                          <div style={{borderTop:'1px solid var(--line)',padding:'14px 16px',background:'rgba(0,0,0,.15)'}}>
                      ...
                      <div style={{padding:'40px',textAlign:'center',border:'1px solid var(--line)',borderRadius:'7px',color:'var(--muted2)',fontFamily:'"JetBrains Mono"',fontSize:'10px',lineHeight:1.9}}>
                        No query probes in this scan.<br/>
                        Add <span style={{color:'var(--lime)'}}>query_probes</span> to the API response schema:<br/>
                        <span style={{color:'var(--muted)',fontSize:'9px'}}>{'[{engine, query, response, cited}]'}</span>
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                      ...
                      <table style={{width:'100%',borderCollapse:'collapse'}}>
                        <thead><tr style={{borderBottom:'1px solid var(--line)'}}>
                          {['Domain','Score','vs You','Bar',''].map(h=>(
                            <th key={h} style={{fontFamily:'"JetBrains Mono"',fontSize:'9px',letterSpacing:'.08em',textTransform:'uppercase',color:'var(--muted2)',padding:'10px 14px',textAlign:'left',fontWeight:400}}>{h}</th>
                          ))}
                        </tr></thead>
                        <tbody>
                          {[{n:resultUrl,s:result.score,isYou:true,gap:''},...result.comps].map((c: any,i)=>(
                            <>
                              <tr key={`row-${c.n}`} onClick={()=>!c.isYou&&setOpenComp(openComp===i?null:i)} className="comp-row"
                                style={{borderBottom:'1px solid rgba(255,255,255,.04)',transition:'.15s',cursor:c.isYou?'default':'pointer'}}>
                                <td style={{padding:'10px 14px',fontFamily:'"JetBrains Mono"',fontSize:'11px',color:c.isYou?'var(--lime)':'var(--text)'}}>
                                ...
                                <td style={{padding:'10px 14px',fontFamily:'"JetBrains Mono"',fontSize:'11px',color:c.isYou?'var(--muted2)':c.s>result.score?'var(--red)':'var(--green)'}}>
                                  {c.isYou?'—':(c.s>result.score?'+':'')+(c.s-result.score)}
                                </td>
                                <td style={{padding:'10px 14px',width:'30%'}}>
                                  <div style={{height:'5px',background:'rgba(255,255,255,.06)',borderRadius:'9px',overflow:'hidden'}}>
                                    <div style={{height:'100%',width:`${c.s}%`,background:c.isYou?'var(--lime)':scolor(c.s),borderRadius:'9px',transition:'width 1s'}} />
                                  </div>
                                </td>
                                <td style={{padding:'10px 14px',fontSize:'10px',color:'var(--muted2)',fontFamily:'"JetBrains Mono"'}}>
                                  {!c.isYou&&(c.gap?<span style={{color:'var(--amber)'}}>{openComp===i?'▲ hide':'▼ gap'}</span>:'—')}
                                </td>
                              </tr>
                              {openComp===i && c.gap && (
                                <tr key={`gap-${c.n}`} style={{borderBottom:'1px solid rgba(255,255,255,.04)'}}>
                      ...
                      return (
                        <div key={i} className="tilt" style={{border:`1px solid ${ps.bc}`,background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px',boxShadow:'inset 0 1px rgba(255,255,255,.045)'}}>
                          ...
                          <div style={{display:'flex',gap:'5px',flexWrap:'wrap'}}>
                            {['High intent','BLUF needed','Schema gap','Competitor edge'].slice(0,f.priority==='HIGH'?3:2).map(t=>(
                              <span key={t} style={{fontFamily:'"JetBrains Mono"',fontSize:'8px',padding:'4px 7px',border:'1px solid var(--line)',borderRadius:'3px',color:'var(--muted)'}}>{t}</span>
                            ))}
                          </div>
                        </div>
                      )
                ...
                {view==='technical' && (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 320px',gap:'12px'}}>
                    <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',overflow:'hidden'}}>
                        <div style={{padding:'14px 16px 10px',borderBottom:'1px solid var(--line)'}}>
                          <div style={{fontFamily:'"Familjen Grotesk"',fontSize:'14px',fontWeight:600}}>Schema + crawlability audit</div>
                        </div>
                        <div style={{padding:'4px 14px 14px'}}>
                          {result.schema.length>0 ? result.schema.map((s,i)=>(
                            <div key={i} style={{display:'grid',gridTemplateColumns:'28px 1fr auto',gap:'10px',padding:'12px 0',borderBottom:'1px solid rgba(255,255,255,.065)',alignItems:'center'}}>
                              <span style={{width:'26px',height:'26px',display:'grid',placeItems:'center',borderRadius:'5px',background:s.status==='pass'?'rgba(82,227,142,.08)':s.status==='fail'?'rgba(255,116,116,.08)':'rgba(255,196,92,.08)',color:s.status==='pass'?'var(--green)':s.status==='fail'?'var(--red)':'var(--amber)',fontSize:'11px'}}>
                            ...
                            <div style={{padding:'20px',textAlign:'center',color:'var(--muted2)',fontFamily:'"JetBrains Mono"',fontSize:'10px'}}>Add <span style={{color:'var(--lime)'}}>schema[]</span> array to API response</div>
                          )}
                        </div>
                      </div>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'16px'}}>
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
                          ...
                          <div style={{display:'flex',gap:'6px'}}>
                            ...
                            <span className="pill" style={{border:`1px solid ${result.llms_txt.valid?'rgba(82,227,142,.25)':'rgba(255,196,92,.25)'}`,background:result.llms_txt.valid?'rgba(82,227,142,.08)':'rgba(255,196,92,.08)',color:result.llms_txt.valid?'var(--green)':'var(--amber)'}}>
                              {result.llms_txt.valid?'VALID':'INVALID'}
                            </span>
                          </div>
                        </div>
                        {result.llms_txt.content ? (
                          <div style={{padding:'12px',background:'rgba(255,255,255,.025)',border:'1px solid var(--line)',borderRadius:'6px',fontFamily:'"JetBrains Mono"',fontSize:'9px',color:'var(--muted)',lineHeight:1.8,whiteSpace:'pre-wrap',maxHeight:'160px',overflowY:'auto'}}>
                            {result.llms_txt.content}
                          </div>
                        ) : (
                          <div style={{padding:'14px',background:'rgba(202,255,69,.04)',border:'1px solid rgba(202,255,69,.15)',borderRadius:'6px',fontSize:'10px',color:'var(--muted)',lineHeight:1.6}}>
                            No llms.txt found at <span style={{color:'var(--lime)',fontFamily:'"JetBrains Mono"'}}>{resultUrl}/llms.txt</span>
                          </div>
                        )}
                        <button onClick={()=>showToast('llms.txt copied!')} style={{marginTop:'10px',width:'100%',border:0,background:'var(--lime)',color:'#07100b',padding:'10px',borderRadius:'5px',fontSize:'11px',fontWeight:700,fontFamily:'"Familjen Grotesk"'}}>
                          {result.llms_txt.exists?'Copy llms.txt':'Generate + copy llms.txt'}
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'15px'}}>
                        ...
                        {[{l:'Experience',v:result.eeat.experience,c:'var(--lime)'},{l:'Expertise',v:result.eeat.expertise,c:'var(--violet)'},{l:'Authority',v:result.eeat.authority,c:'var(--cyan)'},{l:'Trust',v:result.eeat.trust,c:'var(--rose)'}].map(b=>(
                          <div key={b.l} style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}>
                            ...
                            <div style={{flex:1,height:'4px',background:'rgba(255,255,255,.06)',borderRadius:'9px',overflow:'hidden'}}>
                              <div style={{height:'100%',width:`${b.v}%`,background:b.c,borderRadius:'9px',transition:'width 1s'}} />
                            </div>
                      ...
                      <div style={{border:'1px solid var(--line)',background:'linear-gradient(145deg,rgba(14,25,40,.94),rgba(7,13,23,.92))',borderRadius:'7px',padding:'15px'}}>
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                          <span style={{fontSize:'10px',color:'var(--muted)'}}>Content score</span>
                          <span style={{fontFamily:'"Familjen Grotesk"',fontSize:'18px',fontWeight:700,color:scolor(result.bluf.score)}}>{result.bluf.score}</span>
                        </div>
                        <div style={{height:'4px',background:'rgba(255,255,255,.06)',borderRadius:'9px',overflow:'hidden',marginBottom:'12px'}}>
                          <div style={{height:'100%',width:`${result.bluf.score}%`,background:scolor(result.bluf.score),borderRadius:'9px',transition:'width 1s'}} />
                        </div>
                        {result.bluf.headline && (
                          <div style={{padding:'8px 10px',background:'rgba(255,255,255,.03)',borderRadius:'5px',fontSize:'9px',color:'var(--muted)',lineHeight:1.6,marginBottom:'10px',fontStyle:'italic',border:'1px solid var(--line)'}}>"{result.bluf.headline}"</div>
                        )}
                        {result.bluf.issues.map((issue,i)=>(
                          <div key={i} style={{display:'flex',gap:'6px',fontSize:'9px',color:'var(--muted)',marginBottom:'5px',lineHeight:1.5}}>
