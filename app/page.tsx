...
const TOOLS = [
  {
    ...
    cta:'Run free scan', href:'/ai-visibility-tool', tag:'Core', c:'rgba(200,242,71,.2)',
  },
  {
...
const PLANS = [
  ...
  { plan:'Pro', price:'0', desc:'For SEO professionals managing client sites and their own brands.', features:['10 domains included','All 6 LLMs tracked daily','Full citation context + sentiment','Competitor benchmarking (up to 5)','Content gap analysis','Weekly email digest reports','90-day history','CSV/PDF exports'], cta:'Get started free', href:'/ai-visibility-tool', featured:true },
  { plan:'Agency', price:'0', desc:'For agencies managing multiple client accounts at scale.', features:['Unlimited domains','All 6 LLMs + custom prompt testing','White-label reports for clients','Unlimited competitor tracking','API access (coming soon)','Priority Slack support','365-day history'], cta:'Contact sales', href:'/resources/contact', featured:false },
]
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center'}}>
              <div style={{width:'360px',height:'360px',position:'relative',perspective:'900px',display:'flex',alignItems:'center',justifyContent:'center',animation:'floatY 7s ease-in-out infinite'}}>
                ...
                {[{label:'ChatGPT',top:'-10px',left:'50%',transform:'translateX(-50%)'},{label:'Perplexity',top:'50%',right:'-20px',transform:'translateY(-50%)'},{label:'Gemini',bottom:'-10px',left:'50%',transform:'translateX(-50%)'},{label:'Claude',top:'50%',left:'-20px',transform:'translateY(-50%)'}].map((l,i)=>(
                  <div key={i} style={{position:'absolute',top:(l as any).top,bottom:(l as any).bottom,left:(l as any).left,right:(l as any).right,transform:l.transform,fontFamily:"'JetBrains Mono',monospace",fontSize:'.6rem',color:'var(--muted)',padding:'.3rem .6rem',background:'rgba(9,17,28,.9)',border:'1px solid var(--border)',borderRadius:'4px',whiteSpace:'nowrap'}}>{l.label}</div>
                ))}
              </div>
            </div>
          </div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'.55rem',marginBottom:'1.25rem'}}>
                  {card.items.map((item,ii)=>(
                    <div key={ii} style={{display:'flex',alignItems:'flex-start',gap:'.6rem',fontSize:'.82rem',color:'var(--muted)',lineHeight:1.5}}>
                      <span style={{flexShrink:0,color:item.s==='pass'?'#4ade80':item.s==='fail'?'#f87171':'var(--accent)',marginTop:'.05rem'}}>{item.s==='pass'?'✓':item.s==='fail'?'✗':'△'}</span>
                      {item.t}
                    </div>
                  ))}
                </div>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:'16px',padding:'2rem'}}>
              ...
              <div style={{display:'flex',justifyContent:'center',marginBottom:'1.5rem'}}>
                <svg viewBox="0 0 280 280" style={{width:'240px',height:'240px'}}>
                  <defs><radialGradient id="radarFill" cx="50%" cy="50%"><stop offset="0%" stopColor="#7b6cff" stopOpacity="0.25"/><stop offset="100%" stopColor="#c8f247" stopOpacity="0.08"/></radialGradient></defs>
                  ...
                  <circle cx="140" cy="62" r="4" fill="#c8f247"/><circle cx="218" cy="116" r="4" fill="#c8f247"/><circle cx="189" cy="206" r="4" fill="#c8f247"/><circle cx="91" cy="206" r="4" fill="#c8f247"/><circle cx="62" cy="116" r="4" fill="#c8f247"/>
        ...
        <section id="results" style={{padding:'7rem 3.5rem',maxWidth:'1320px',margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1rem',marginBottom:'4rem'}}>
            {[
              ...
              <div key={i} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:'14px',padding:'2rem',display:'flex',flexDirection:'column',gap:'1.25rem'}}>
